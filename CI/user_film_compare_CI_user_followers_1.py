...
# Else go through following or followers
	...
	newouttxt = 1
	newoutcsv = 1
	outpathtxt = Path('trash')
	outpathcsv = Path('trash')
	...
	if outpathcsv.exists():
		...
		if outcsvchoice != "y":
			newoutcsv = 0
	if newouttxt == 1:
		outfile = open('CI/user_film_compare_CI_user_followers_1.txt','w')
		for i in range(len(sortedusers)):
			outfile.write('{:{longest}} {:8.5f}  {}{}'.format(sortedusers[i],scores[i],sortedinterpretations[i],'\n',longest=longest))
		# Close output file:
		outfile.close()
	if newoutcsv == 1:
		with open('CI/user_film_compare_CI_user_followers_1.csv', mode='w') as outfile:
			csvwriter = csv.writer(outfile, delimiter=',', quotechar='"', quoting=csv.QUOTE_MINIMAL)
			for i in range(len(sortedusers)):
